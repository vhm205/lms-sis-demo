...
export function ScheduleClient({ schedules, classes, rooms, teachers }: { schedules: any[], classes: any[], rooms: any[], teachers: any[] }) {
  ...
  const { selectedFacilityId, selectedFacility, setSelectedFacilityId } = useFacility();
  ...
  const filtered = schedules.filter(s => {
    ...
    if (!term) return matchFacility(s);

    const dateObj = new Date(s.date);
    const dayStr = dateObj.toLocaleDateString("vi-VN", { weekday: "short", day: "2-digit", month: "2-digit", year: "numeric" }).toLowerCase();
    const timeStr = dateObj.toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" }).toLowerCase();

    const matchSearch = 
      s.class?.name?.toLowerCase().includes(term) ||
      s.class?.code?.toLowerCase().includes(term) ||
      s.class?.course?.name?.toLowerCase().includes(term) ||
      s.room?.name?.toLowerCase().includes(term) ||
      (s.class?.teacher?.name && s.class.teacher.name.toLowerCase().includes(term)) ||
      (s.class?.students && s.class.students.some((st: any) => st.name?.toLowerCase().includes(term) || st.code?.toLowerCase().includes(term))) ||
      dayStr.includes(term) ||
  ...
  const {
    ...
    getId: (s) => s.id,
    getSecondaryId: (s) => s.classId,
    getFacilityId: (s) => s.class?.facilityId || s.room?.facilityId,
          ...
          <TableBody>
            {paginatedSchedules.length === 0 ? (
              ...
              paginatedSchedules.map((s) => {
                ...
                const isRowHighlighted = isHighlighted(s);

                return (
                  <TableRow 
                    key={s.id} 
                    id={`row-highlight-${s.id}`}
                    className={cn(
                    ...
                    <TableCell>
                      <div className="space-y-1 py-1">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-sm text-foreground font-heading">{s.class?.name}</span>
                          {isRowHighlighted && (
