...
import { createLead, updateLead, updateLeadStatus, deleteLead } from "@/app/actions/lead";
...
export function LeadsClient({ leads, courses, facilities }: { leads: any[], courses: any[], facilities: any[] }) {
  ...
  const {
    ...
    getId: (l) => l.id,
    getFacilityId: (l) => l.facilityId || l.facility?.id,
        ...
        <div className="flex items-center justify-between px-4 py-3 rounded-2xl bg-[#FFF0E6] dark:bg-[#352114] border-2 border-[#FCDCC8] dark:border-[#523824] text-xs font-bold text-[#D97736] dark:text-[#FBAA78] shadow-sm animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <span className="truncate">
              Đang làm nổi bật khách tiềm năng được chọn từ tìm kiếm nhanh:{" "}
              <span className="font-extrabold underline font-heading text-foreground">{highlightedItem.name}</span>{" "}
              <span className="font-mono text-[11px] font-bold">({highlightedItem.phone})</span>
            </span>
          </div>
          <button
            type="button"
          ...
          <TableBody>
            {paginatedLeads.length === 0 ? (
              ...
              paginatedLeads.map((lead) => {
                const isRowHighlighted = isHighlighted(lead);

                return (
                  <TableRow 
                    key={lead.id} 
                    id={`row-highlight-${lead.id}`}
                    className={cn(
