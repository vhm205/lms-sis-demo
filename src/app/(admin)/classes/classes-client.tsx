...
import { createClass, updateClass, deleteClass } from "@/app/actions/class";
...
export function ClassesClient({ classes, courses, facilities, teachers }: { classes: any[], courses: any[], facilities: any[], teachers: any[] }) {
  ...
  const {
    ...
    getId: (c) => c.id,
    getSecondaryId: (c) => c.code,
    getFacilityId: (c) => c.facilityId || c.facility?.id,
        ...
        <div className="flex items-center justify-between px-4 py-3 rounded-2xl bg-[#FFF0E6] dark:bg-[#352114] border-2 border-[#FCDCC8] dark:border-[#523824] text-xs font-bold text-[#D97736] dark:text-[#FBAA78] shadow-sm animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <span className="truncate">
              Đang làm nổi bật lớp học được chọn từ tìm kiếm nhanh:{" "}
          ...
          <TableBody>
            {paginatedClasses.length === 0 ? (
              ...
              paginatedClasses.map((cls) => {
                ...
                const isRowHighlighted = isHighlighted(cls);

                return (
                  <TableRow 
                    key={cls.id} 
                    id={`row-highlight-${cls.id}`}
                    className={cn(
